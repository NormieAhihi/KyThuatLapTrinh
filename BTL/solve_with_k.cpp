...
string statement_k[2] = {"\t\tSai so tuyet doi tien nghiem: ", "\t\tSai so tuyet doi hau nghiem: "};
...
void solve_iterative_single_row_k(double **B, double *d, int k, int type) {
    ...
    temp = k;
    while(temp > 0) {
    	cout << "\t\tLan lap thu " << k - temp << ": x[" <<  k - temp << "]= ";
    	out << "\t\tLan lap thu " << k - temp << ": x[" <<  k - temp << "]= ";
		print_vector1(x); 
        temp = temp - 1;
    ...
    cout << fixed << statement_k[type-1] << setprecision(precision) << error << endl;
    cout << "\t\tNghiem cua he phuong trinh: \n";
    
    out << fixed << statement_k[type-1] << setprecision(precision) << error << endl;
